...
def load_entity(client, item_id):
    ...
    logging.info('retrieved entity for ' + str(item_id))
    return entity
...
def get_list_items():
    ...
    logging.info('list retrieved. %s items' % (len(result)))
    return result
...
def create_list_item(shopping_list_item):
    ...
    client.put(entity)
    logging.info('saved new entity for ID: %s' % (key.id_or_name))
...
def save_list_item(shopping_list_item):
    ...
    client.put(entity)
    logging.info('entity saved for ID: %s' % (shopping_list_item.id))


def get_list_item(sli_id):
    """Retrieve an object for the ShoppingListItem with the specified ID."""
    client = get_client()
    logging.info('retrieving object for ID: %s' % (sli_id))
...
def delete_list_item(sli_id):
    ...
    logging.info('key loaded for ID: %s' % (sli_id))
    client.delete(key)
    logging.info('key deleted for ID: %s' % (sli_id))
