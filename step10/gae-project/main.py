...
def get_things(sort=None, startswith=None):
    result = []
    client = get_client()
    ...
    if startswith:
        query.add_filter('text', '>=', startswith)
        query.add_filter('text', '<', startswith + '~')
...
def show_things(sort=None):
    things_list = get_things(sort)
...
def handle_request_root():
    ...
    if 'sort' in flask.request.values:
        sort_order = flask.request.values['sort']
    return show_things(sort_order)
...
@app.route('/filter-things', methods=['POST', 'GET'])
def handle_filter_things():
    startswith = flask.request.values['startswith']
    things_list = get_things(startswith=startswith)
    return flask.render_template('index.html', things=things_list)
